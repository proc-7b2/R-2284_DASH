...
with Cols_Left:
    display_df = filtered_data[['rank','Id', 'name', 'creatorName', 'Image Url', 'snapDate','Created','creatorType','creatorHasVerifiedBadge','favoriteCount']].copy()
...
with Cols_Right:
    if len(event.selection.rows) > 0:
        ...
        with st.container(border=True):
            ...
            st.image([selected_data['Image Url']], width=400)
        ...
        with st.container(border=True): 
                ...
                if pd.notna(fav_raw):
                    ...
                    fav_display = "N/A"
                st.write(f"**Favorites:** {fav_display}")
